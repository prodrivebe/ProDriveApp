...
WORKFLOW_TRANSITIONS: dict[OrderStatus, set[OrderStatus]] = {
    ...
    OrderStatus.IN_TRANSIT: {OrderStatus.LOADED, OrderStatus.DELIVERING},
...
STOP_PROGRESS_TRANSITIONS: dict[StopProgressStatus, set[StopProgressStatus]] = {
    ...
    StopProgressStatus.COMPLETED: {StopProgressStatus.LOADING},
}
...
NEXT_REQUIRED_ACTIONS: dict[OrderStatus, str] = {
    ...
    OrderStatus.ARRIVED_DELIVERY: "Start delivery",
    OrderStatus.DELIVERING: "Complete delivery",
...
def get_active_stops(stops: list[OrderStop]) -> list[OrderStop]:
    """Return non-deleted stops sorted by sequence."""
    active = [stop for stop in stops if stop.deleted_at is None]
    active.sort(key=lambda item: item.sequence)
    return active


def get_current_stop(stops: list[OrderStop]) -> OrderStop | None:
